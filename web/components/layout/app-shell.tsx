...
/**
 * Shell padrão das páginas pós-login do TENANT (chat / cobranças / usuários).
 *
 * Header sempre visível. Hamburger só aparece pra `role === "admin"` —
 * outros roles têm header simples sem menu de navegação.
 *
 * Visibilidade dos itens da sidebar:
 *   - Bella Chat   → tenant tem `chat` contratado
 *   - Bella Cobranças → tenant tem `cobrancas` contratado
 *   - Usuários → sempre (rota só serve admin de qualquer jeito)
 */
export function AppShell({
  ...
  const items: SidebarItem[] = [
    ...
    },
    {
      ...
      label: "Usuários",
      icon: Users,
      visible: true, // backend já bloqueia se não for admin
    },
  ];
